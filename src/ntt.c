...
}; C'EST CELLE UTILISEE DANS LE DOMAINE DE MONTGOMERY */
...
void NTT_inv(int16_t f[256]) {
    ...
    for (j = 0; j < KYBER_N; j++) {
        /*
         * The inverse NTT need a final normalization which consists of multiplying by 128^{-1} = 3303.
         * Since we chose to do all the NTT computations inside the Montgomery domain, we shall also convert
         * the above constant into the Montgomery domain. This means replacing it by 128^{-1} * 2^16 = 512.
         */
